...
def ConvDiscriminator(input_shape=(256, 256, 3),
	...
	x = tf.keras.layers.Conv2D(dim, 4, strides=2, padding='same')(inputA)
	x = tf.nn.leaky_relu(x, alpha=0.2)
	x = Norm()(x)
		
	y = tf.keras.layers.Conv2D(dim, 4, strides=2, padding='same')(inputB)
	y = tf.nn.leaky_relu(y, alpha=0.2)
	y = Norm()(y)
		
	z = tf.keras.layers.Conv2D(dim, 4, strides=2, padding='same')(inputC)
	...
	#4
	#h = tf.keras.layers.AveragePooling2D(pool_size=(1, 1), strides = 3, padding='same')(h)
	
	return keras.Model(inputs = [inputA, inputB, inputC], outputs = h)
#return keras.Model(inputs = inputs, outputs=h)
#'''
#return keras.Model(inputs =  z.input, outputs=h)
