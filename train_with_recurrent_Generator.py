...
@tf.function
def insertTemporalInformation(pImg,f):
	return tf_warp(pImg,f)
...
def train_G(A, B):
    with tf.GradientTape() as t:
		...
		A2B = insertTemporalInformation(A2B,f) #Modification Here 
		B2A = G_B2A(B, training=True)
		A2B = insertTemporalInformation(A2B,f)
...
with train_summary_writer.as_default():
	for ep in tqdm.trange(args.epochs, desc='Epoch Loop'):
		...
		for A, B in tqdm.tqdm(A_B_dataset, desc='Inner Epoch Loop', total=len_dataset):
			...
			if G_optimizer.iterations.numpy() % 2 == 0:
				A, B = next(test_iter)
				#checkpoint.save(ep)
